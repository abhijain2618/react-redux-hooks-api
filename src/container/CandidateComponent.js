...
const CandidateComponent = () => {
  ...
  console.log('query param', filterParam);  
  const dispatch = useDispatch();
  const candidates = useSelector((state) => state.candidateReducer);  
  const renderList = (input) => {if (input) return input.map((candidate) => {
    const {
      id,
      name,
      email,
  ...
  const  search = () => {
    console.log('row searching');
    if(searchBy != ""){
      return candidates.filter((el) =>  el.name.toLowerCase().indexOf(searchBy) >-1 );          
    }else{
      return candidates;      
    }
  }
      ...
      <table className="ui celled table">
        <thead>
          <tr>
            ...
            <th>Email</th>
            <th >DOB</th>
            <th onClick={() => {sortBy(SORT_BY.YEARS_OF_EXPERIENCE)}}>Years of Experience<i className="sort icon"></i></th>
            <th onClick={() => {sortBy(SORT_BY.POSITION_APPLIED)}}>Position Applied<i className="sort icon"></i></th>
            <th onClick={() => {sortBy(SORT_BY.DATE_OF_APPLICATION)}}>Application Date<i className="sort icon"></i></th>
            <th>Status</th>
          </tr>
        </thead>
